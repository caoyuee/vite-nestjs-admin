import { type VNode, type ComponentPublicInstance, type Ref } from "vue";
...
export type SearchParam = Record<string, unknown>;
export type EnumValue = string | number | boolean | unknown[];

export interface EnumProps {
  label?: string; // 选项框显示的文字
  value?: EnumValue; // 选项框值
...
export type SearchProps = {
  ...
  props?: Record<string, unknown>; // 搜索项参数，根据 element plus 官方文档来传递，该属性所有值会透传到组件
  key?: string; // 当搜索项 key 不为 prop 属性时，可通过 key 指定
  tooltip?: string; // 搜索提示
  order?: number; // 搜索项排序（从大到小）
  span?: number; // 搜索项所占用的列数，默认为 1 列
  offset?: number; // 搜索字段左侧偏移列数
  defaultValue?: unknown; // 搜索项默认值
...
export interface ColumnProps<T extends DefaultRow = DefaultRow>
  ...
  enum?: unknown[] | Ref<unknown[]> | ((params?: never) => Promise<{ data: unknown[] } | unknown[]>); // 枚举字典
